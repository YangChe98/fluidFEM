function [A11reference,A12reference,A22reference]=referencematrixA(phi,phix,phiy)
...
for i=1:12
    for j=1:12
        A11=int(matlabFunction(phix(1,i).*phix(1,j)+phix(2,i).*phix(2,j),'vars',[x y]),x,-1,1);
        A1=int(matlabFunction(A11,'var',[y]),y,-1,1);
        A11reference(i,j)=double(A1);
%         
      

        A22=int(matlabFunction(phiy(1,i).*phiy(1,j)+phiy(2,i).*phiy(2,j),'vars',[x y]),x,-1,1);
        A2=int(matlabFunction(A22,'var',[y]),y,-1,1);
        A22reference(i,j)=double(A2);
  
          A12=int(matlabFunction(phix(1,i).*phiy(1,j)+phix(2,i).*phiy(2,j),'vars',[x y]),x,-1,1);
